...
export default function Learn() {
  ...
  // 페이지 진입 시 안내
  useEffect(() => {
    const message = "점자 학습 모드입니다. 자모 학습, 단어 학습, 문장 학습, 자유 변환 중에서 선택해주세요.";
    speak(message);
  }, [speak]);
          ...
          >
            <div className="flex items-center justify-between mt-3">
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                mode.level === '기초' ? 'bg-primary text-black' :
                mode.level === '중급' ? 'bg-accent text-black' :
                mode.level === '고급' ? 'bg-danger text-white' :

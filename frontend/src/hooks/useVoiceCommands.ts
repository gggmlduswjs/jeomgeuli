...
function normalize(raw: string): string {
  ...
  t = t.replace(/자유변화/gi, "자유변환");
  
  // 페이지 이동 명령
  t = t.replace(/학습모드|학습 모드/gi, "학습");
  t = t.replace(/자유변환|자유 변환/gi, "자유변환");
  ...
  // 재생 제어 명령
  t = t.replace(/다음으로가|다음으로 가/gi, "다음");
  t = t.replace(/이전으로가|이전으로 가/gi, "이전");
...
const COMMAND_PHRASES: Record<string, string[]> = {
  ...
  freeConvert: ["자유변환", "자유 변환", "변환", "점자변환", "점자 변환", "변환해", "자유변환해", "자유 변환해"],
  ...
  // 재생 제어
  next: ["다음", "넘겨", "다음으로", "계속", "진행", "다음해", "넘겨줘", "계속해", "진행해"],
...
function matchCommand(text: string, threshold: number = 0.40): string | undefined {
  ...
  // 임계값 이상이면 명령 반환 (더 관대하게 0.40으로 낮춤)
  if (bestScore >= threshold) {
    console.log(`[VoiceCommands] 퍼지 매칭: "${text}" -> "${bestCmd}" (유사도: ${(bestScore * 100).toFixed(1)}%)`);
    return bestCmd;
  }

...
export function BraillePanel({ braille }: BraillePanelProps) {
  const { status, currentWord, currentCells, demoMode, queue, next, repeat, pause, enqueueKeywords } = braille;

  return (
    <section className="braille-panel bg-white border border-slate-200 rounded-lg p-4 mb-4">
      {/* 패널 헤더 */}
      <div className="panel-header flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className={`badge px-2 py-1 rounded-full text-xs font-medium ${
            status.includes('출력 중') ? 'bg-green-100 text-green-700' :
            status.includes('일시 정지') ? 'bg-yellow-100 text-yellow-700' :
      ...
      <div className="preview text-center">
        ...
        <div className="cells flex justify-center gap-2 mb-3">
          {currentCells?.length ? (
            currentCells.map((pattern, i) => (
              <BrailleCell 
                key={i} 
                pattern={pattern} 
                active={status.includes('출력 중')}
                className="scale-110"
                aria-label={`점자 셀 ${i+1}`} 
              />
            ))
